...
class Revo_pos_authentication {
	...
	// 	$woocommerce = ( false !== strpos( $request_uri, $rest_prefix . 'revo-admin/' ) );

	// 	// Allow third party plugins use our authentication methods.
	// 	$third_party = ( false !== strpos( $request_uri, $rest_prefix . 'revo-admin-' ) );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $params ) ) {
			$this->set_error( new WP_Error( 'revo_rest_authentication_error', __( 'Consumer key is invalid.', 'woocommerce' ), array( 'status' => 401 ) ) );
			return false;
		}
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new WP_Error( 'revo_rest_authentication_error', __( 'Invalid signature - failed to sort parameters.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			return new WP_Error( 'revo_rest_authentication_error', __( 'Invalid signature - signature method is invalid.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) { // @codingStandardsIgnoreLine
			return new WP_Error( 'revo_rest_authentication_error', __( 'Invalid signature - provided signature does not match.', 'woocommerce' ), array( 'status' => 401 ) );
		}
