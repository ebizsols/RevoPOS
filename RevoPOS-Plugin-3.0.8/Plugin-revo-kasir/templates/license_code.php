<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $cek_code = pos_revo_pos_cek_license_code($data);
      if ($cek_code->status == 'success') {
        $data = [
          'description' => json_encode($cek_code->data),
          'update_at' => $cek_code->data->expired_at,
          'slug' => 'revo_pos_license_code'
        ];
        $cek = $wpdb->get_row("select * from revo_pos_mobile_variable where slug='revo_pos_license_code'");

        if(empty($cek)){
            $wpdb->insert('revo_pos_mobile_variable',$data);
        }else{
            $wpdb->update('revo_pos_mobile_variable',$data,['slug' => 'revo_pos_license_code']);
        }
        ...
        if (@$wpdb->insert_id > 0) {
          ...
          wp_redirect( admin_url( '/admin.php?page='.REVO_POS_PLUGIN_SLUG ) );
          exit;
        }


        echo "<script>window.location.href='".admin_url( '/admin.php?page='.REVO_POS_PLUGIN_SLUG)."';</script>";
      }else{
        $alert = array(
              'type' => 'error',
